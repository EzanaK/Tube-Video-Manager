...
public class TubeVideosManager implements TubeVideosManagerInt {
	...
	public ArrayList<Video> getAllVideosInDB() {
		...
		int i;
		for (i = 0; i < this.videoDatabase.size(); i++) {
			list.add(this.videoDatabase.get(i));
		}
		return list;
	}
	...
	public Video findVideo(String title) {
		if (title == null || title.isBlank()) {
			...
			int i;
			for (i = 0; i < this.videoDatabase.size(); i++) {
				if (this.videoDatabase.get(i).getTitle().equals(title)) {
					Video video = new Video(this.videoDatabase.get(i));
					return video;
				}
			}
			return null;
	...
	public boolean addComments(String title, String comments) {
		if (title == null || title.isBlank() || comments == null || comments.isBlank()) {
			...
			for (i = 0; i < this.videoDatabase.size(); i++) {
				if (this.videoDatabase.get(i).getTitle().equals(title)) {
					this.videoDatabase.get(i).addComments(comments);
					return true;
				}
			}
			return false;
	...
	public boolean addPlaylist(String playlistName) {
		if (playlistName == null || playlistName.isBlank()) {
			...
			for (i = 0; i < this.playlists.size(); i++) {
				if (this.playlists.get(i).getName().equals(playlistName)) {
	...
	public String[] getPlaylistsNames() {
		int playlistSize = this.playlists.size();
		String[] strArray = new String[playlistSize];
		int i;
		for (i = 0; i < playlistSize; i++) {
			strArray[i] = this.playlists.get(i).getName();
		}
		return strArray;
	...
	public boolean addVideoToPlaylist(String title, String playlistName) {
		...
		if (this.findPlayListInternal(playlistName) != null) {
	...
	public Playlist getPlaylist(String playlistName) {
		if (playlistName == null || playlistName.isBlank()) {
			...
			if (this.findPlayListInternal(playlistName) != null) {
				Playlist playlist = new Playlist(this.findPlayListInternal(playlistName));
				return playlist;
			} else {
				return null;
			}
	...
	public Playlist searchForVideos(String playlistName, String title, int maximumDurationInMinutes, Genre genre) {
		...
		int i;
		for (i = 0; i < this.videoDatabase.size(); i++) {
			if (title == null || title.equals(this.videoDatabase.get(i).getTitle())) {
				if (maximumDurationInMinutes < 0
						|| this.videoDatabase.get(i).getDurationInMinutes() <= maximumDurationInMinutes) {
					if (genre == null || genre.name().equals(this.videoDatabase.get(i).getGenre().name())) {
						playlist.addToPlaylist(this.videoDatabase.get(i).getTitle());
					}
				}
			}
		}
